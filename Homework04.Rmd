...
---
Repository: (https://github.com/jynakay/Assignments)[https://github.com/jynakay/Assignments] 
...
Extra credit: What is the difference if you use the `paste0` function instead of the `paste` function above? **`paste0` does not add a space between merged text, whereas `paste` does add a space.**
...
last5letters <- str_sub(herName, Length-5, Length)
last5letters
```
...
```{r Answer 5}
myNewString <- "Now_is_the_time_for_all_good_men_to_come_to_the_aid_of_their_country"
myNewSeparatedString <- str_split(myNewString, "_")
myNewSeparatedString
